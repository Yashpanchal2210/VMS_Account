...
namespace VMS_1
{
    public partial class SailorsVAcc : System.Web.UI.Page
    {
        ...
        protected void ExportToExcelButton_Click(object sender, EventArgs e)
        {
            ...
            {
                ExcelWorksheet worksheet = excelPackage.Workbook.Worksheets.Add("Sailor Report");
                ...
                worksheet.Cells["B4"].Value = "RIK + HON. Sailor";
                ...
                {
                    worksheet.Cells["C3"].Value = OfficerGEN.Rows[0]["nonVegSailor"];
                    worksheet.Cells["D3"].Value = "";
                    worksheet.Cells["E3"].Value = OfficerGEN.Rows[0]["vegSailor"];
                ...
                {
                    worksheet.Cells["C4"].Value = OfficerRik.Rows[0]["nonVegSailorRik"];
                    worksheet.Cells["D4"].Value = "";
                    worksheet.Cells["E4"].Value = OfficerRik.Rows[0]["vegSailorRik"];
                ...
                string fileName = $"Sailor_{DateNow}.xlsx";
        ...
        private DataTable GetOfficeBasicItemsData()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                using (SqlCommand cmd = new SqlCommand("select * from BasicItems WHERE Category='Sailor'", conn))
                {
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    adapter.Fill(dataTable);
                }
        ...
        private DataTable GetOfficeInLieuItemsData(int BasicItemId)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                using (SqlCommand cmd = new SqlCommand("select * from InLieuItems WHERE Category='Sailor' AND BasicItemId=@BasicItemId", conn))
                {
        ...
        private DataTable GetOfficeIssueItemsData(int BasicItemId)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT IM.ItemName,SUM(convert(decimal,IM.QtyIssued))QtyIssued FROM IssueMaster IM INNER JOIN InLieuItems INL ON  IM.ItemId=INL.Id WHERE INL.BasicItemId=@BasicItemId AND IM.Role='Galley' GROUP BY month(IM.Date),IM.ItemName", conn))
                {
                    cmd.Parameters.AddWithValue("@BasicItemId", BasicItemId);
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    adapter.Fill(dataTable);
